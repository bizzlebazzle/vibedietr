...
class WebpageRecipeImportTest extends TestCase
{
    ...
    public function test_job_creates_one_private_source_attributed_draft_and_retry_is_idempotent(): void
    {
        ...
        $this->assertSame('240.000000000000000000', $import->nutrition_source['per_serving']['energy_kcal']['value']);
        $this->assertSame($import->id, $import->nutrition_source['provenance']['recipe_import_id']);
        $this->assertSame($import->extractor_version, $import->nutrition_source['provenance']['extractor_version']);
        $this->assertSame('calories', $import->nutrition_source['observations']['energy_kcal']['source_field']);
        $snapshot = app(RecipeVersionContent::class)->snapshot($recipe);
        $this->assertSame($import->id, $snapshot['imported_nutrition']['provenance']['recipe_import_id']);
        $this->assertSame('240.000000000000000000', $snapshot['imported_nutrition']['per_serving']['energy_kcal']['value']);
