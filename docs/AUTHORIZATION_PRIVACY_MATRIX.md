...
It has no production administrator bootstrap or lifecycle and no recipe,
...
- No production assignment path exists; bootstrap and lifecycle remain FND-14.
...
- Controller show, edit, update, and delete operations invoke the policy. The
  Livewire index modal authorizes view/update and the show component authorizes
  view. `App\Livewire\Ingredients\Form::save()` does not authorize its update
  mutation itself; it relies on an authorized entry path, so mutation-boundary
  enforcement is incomplete.
- Ingredient creation assigns the authenticated user's identifier. The
  Livewire form payload also contains `user_id`, including during update, so a
  future implementation must prevent forged ownership reassignment at the
  mutation boundary.
...
| Current user-owned ingredients | Current legacy resource | `ingredients.user_id` user | Same user | Owner only | Denied by authenticated routes | Denied by owner policy/query scope | Current: create, view, edit, and permanently delete | Denied | No administrator role or override exists | Authenticated user; current store paths assign authenticated user | Owner only | Owner only, but Livewire update lacks its own mutation-boundary authorization | Owner only; hard delete | Denied | Denied by intended ownership rule; Livewire payload risk must be fixed by STB-03 | Current: database cascade permanently deletes all rows immediately with account | No audit store exists | None | STB-01, STB-03, FND-02 | This row documents legacy behavior, not the target shared catalogue. Restore and force-delete policy actions are denied. |
...
FND-13 now implements the second-factor records and notification-intent/
provider-acceptance foundations described above. Authenticated, active,
email-verified users may manage only their own pending enrollment and recovery
material; enrollment grants no role. Guests and other users are denied by the
authenticated owner boundary. FND-13 does not expose a security audit browser,
another account's destination or delivery state, or any FND-14 bootstrap,
promotion, revocation or break-glass privilege transition.
