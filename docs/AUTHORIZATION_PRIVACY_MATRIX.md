...
| Bookmarks or saved recipes | Current private recipe bookmarks; organisation remains planned | Bookmarking user | Bookmarking user | Owner only | Denied | Denied | Allowed: create, list, and remove; private organisation remains REC-12 | Denied | Denied unless acting as owner | Authenticated user may idempotently bookmark a currently public finalized durable recipe; ownership is server-derived | Owner only; live recipe content is resolved separately through REC-06 | No content edit; future organisation is owner only | Owner only; repeated removal is safe | Cannot share a bookmark; it is a pointer and copies no recipe content | Denied | Permanently removed with bookmark owner's private data; source deletion leaves the owner's bookmark tombstone | Not required for ordinary add/remove | DEC-012, DEC-013 | REC-10, REC-12, DEP-07, DEP-08 | The indexed durable recipe ID intentionally has no source foreign key. Private, missing, or deleted sources remain generic tombstones; re-publication restores existing pointers; active drafts never appear. |
