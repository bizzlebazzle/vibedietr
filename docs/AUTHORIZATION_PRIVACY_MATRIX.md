...
| Import processing records and job metadata | Current for pasted-text parsing | Importing user for access; application for operation | Application job acting for importing user | Owner only for user-facing status; operational details restricted | Denied | Denied | Allowed: view own status/result and invoke bounded retry; cancellation is not implemented | Denied | Minimized source-free operations metadata only; no private content | Created only for an authorized user's import | Owner sees own status and safe failure category | User cannot rewrite provenance/job history; retry advances controlled state on the same import | Terminal operational metadata is deleted 30 days after completion | Denied | Denied | User-facing record removed with import/account; minimal terminal operational metadata is deleted after 30 days under DEC-013 | Correlation, attempt/failure classification, parser identifier, and opaque import identity are recorded without input text | DEC-005, DEC-006, DEC-007, DEC-013 | FND-09, REC-15 through REC-17, DEP-04, DEP-05 | REC-15 uses identifier-only queue payloads and FND-09 safe reporting; later import channels remain planned. |
