...
| Pending manual catalogue additions | Current NUT-03 read policy; creation workflow planned | No ordinary-user owner; submitter has temporary private read/use access | Submitting user | Submitter and administrator only | Denied | Denied unless the viewer is submitter | Allowed: view and use while pending; edit, withdraw, delete, provenance reassignment, and self-approval denied | Denied | Allowed: view, mark a candidate distinct, reject as duplicate with a suggested replacement, confirm duplicate evidence, approve, or reject; no decision without administrator authorization | Authenticated user submits a non-barcode item; barcode prohibited | Submitter and administrator only | Submitter denied; administrator does not edit it as user content but records a moderation decision | Submitter withdrawal/delete denied; duplicate rejection retains a non-selectable tombstone and dependent references until owner-confirmed replacement or clearing | Cannot be shared/published; approval makes it shared catalogue data | Not applicable; approval changes catalogue status, not ownership transfer | Pending submission permanently removed after recovery period; approved record follows shared-catalogue row | Submission, review, decision, and dependent-reference outcome are auditable | DEC-009, DEC-010, DEC-011, DEC-013 | NUT-08, NUT-09, FND-04, FND-05, DEP-08 | DEC-011 prohibits automatic merging. Strong deterministic matches require explicit reuse or a distinction explanation; fuzzy matches suggest only. Rejection never silently substitutes another item. |
