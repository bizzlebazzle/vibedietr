...
| Recipe remixes | Current REC-11; public attribution profiles remain planned | Remixer | Remixer; source creator is a nullable internal reference, not displayed attribution before REC-14 | Private draft initially; finalized visibility follows recipe rule | Allowed only after remix is finalized public | Same as any recipe | Remixer has normal owner permissions | Same as recipe/plan-snapshot rules; source creator receives no edit grant | Proposed: same as other recipes | Authenticated viewer may remix an accessible current finalized recipe; one operation replay is idempotent | Follows remix lifecycle and visibility; lineage source content is independently authorized | Remixer alone; lineage is immutable | Remixer alone | Follows normal recipe publication; source link is attribution, not shared authority | Denied; remix creation is independent copying, not transfer | Follows remix's own visibility; source deletion or source-owner deletion does not remove it; creator erasure nulls the internal reference | Remix lineage and exact source version use a minimized `recipe.remixed` event | DEC-012, DEC-013, DEC-018 | REC-11, REC-14, DEP-08 | Remix remains usable if source is unavailable. Inaccessible lineage shows only an unavailable-source tombstone and version number; no account name, email, internal ID, profile data, or inaccessible source title/link is displayed or copied. Private-source remix is allowed only where the user already has access; exact plan-snapshot-to-remix permission is not specified and is therefore denied by the proposed default. |
