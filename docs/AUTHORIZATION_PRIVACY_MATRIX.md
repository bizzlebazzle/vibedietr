...
- Controller show, edit, update, and delete operations invoke the policy. The
  ...
  mutation itself. Direct invocation by another authenticated user currently
  updates the row and reassigns ownership; guest invocation reaches a database
  constraint error. Mutation-boundary enforcement is therefore incomplete.
...
| Current user-owned ingredients | Current legacy resource | `ingredients.user_id` user | Same user | Owner only | Denied by authenticated routes | Denied by owner policy/query scope | Current: create, view, edit, and permanently delete | Denied | No administrator role or override exists | Authenticated user; current store paths assign authenticated user | Owner only | Intended owner only; controller returns 403, but direct Livewire update currently succeeds without authorization | Owner only; hard delete | Denied | Intended denied; direct Livewire update currently reassigns ownership and must be fixed by STB-03 | Current: database cascade permanently deletes all rows immediately with account | No audit store exists | None | STB-01, STB-03, FND-02 | This row documents legacy behavior, not the target shared catalogue. Restore and force-delete policy actions are denied. |
