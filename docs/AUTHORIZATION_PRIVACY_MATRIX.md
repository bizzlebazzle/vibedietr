...
REC-09 now provides guest and authenticated title and accepted-public-tag
browse/search through the same centralized public-readable scope. Discovery
projects only durable recipe ID, current-version title, servings and
finalization time plus accepted free-form tag wording and managed term
category/name. It does not load or serialize owners, account/security fields,
mutable draft state, suggestions, private tags, active-revision or historical-
version identifiers, or private recipes.
...
| Public attribution profiles | Current REC-14; planned deletion behavior differs | Account user | Account user | No implicit profile record; profile and both listings default disabled | Allowed only when enabled, through opaque profile ULID | Allowed only when enabled, through the same projection | Allowed: select attribution, enable/disable profile, and independently enable recipe/remix listings | Public read-only only; no separate shared-user grant | Same public read as any viewer; no edit unless acting as owner | Authenticated owner only; ownership is server-derived | Enabled profile exposes only ULID, selected attribution name, and enabled current-public recipe/remix lists | Authenticated owner only; target user/profile identifiers are prohibited | No standalone destructive action; current account deletion cascades the profile row | Owner alone enables profile/listings; disabling never changes recipe visibility, URLs, versions, or snapshotted attribution | Denied; attribution choice is not account transfer | Current: immediate account hard delete removes profile and owned recipes. Planned DEP-08: private profile data is removed and retained public recipes use anonymized former-user attribution | Ordinary settings changes have no audit event; future account anonymization remains auditable under DEC-013 | DEC-012, DEC-013 | REC-14, DEP-08 | Email is never accepted or exposed. Publication snapshots the selected label; private account name/email changes do not rewrite it. Disabled profiles are 404 and attribution becomes non-linked text. Draft/private/historical recipes, active revisions, bookmarks, collections, private tags/counts, and account/security data are excluded. |
...
| Recipe ingredient lines, instructions, matches, and recipe nutrition records | Current recipe lines/instructions, manual catalogue matches, finalized nutrition read projection, overrides, and recalculation history | Recipe owner | Recipe creator or import process acting for owner | Inherit draft/version visibility | Allowed only as content of an accessible public recipe/version | Allowed only as content of an accessible recipe or scoped plan snapshot | Allowed as part of own draft; published copies are changed through revision workflow | Read-only snapshot fields needed for a granted plan view; no mutation | Proposed: no private access or direct mutation | Created only inside an owned recipe draft/import | Inherit accessible parent or pinned-snapshot scope | Owner only in draft; no direct edit of immutable published version | Deleted with removable parent only when snapshots, lineage, and audit integrity remain intact | Cannot be independently shared or published | Not applicable independently | Follows parent recipe/version; retained snapshots disclose only the fields their plan access permits | Original text, matches/provenance, overrides, calculations, and version inputs require history/audit as specified | DEC-001, DEC-002, DEC-003, DEC-004, DEC-013 | REC-02, REC-03, REC-07, NUT-07, NUT-12, NUT-15 through NUT-18, PLAN-03 | Original ingredient and instruction wording is preserved. Recalculation never rewrites a recipe, plan, or diary snapshot. Nested identifiers must not bypass parent authorization. |
| Recipe remixes | Current REC-11 and REC-14 attribution | Remixer | Remixer; source creator remains a nullable internal reference while selected attribution is version-snapshotted | Private draft initially; finalized visibility follows recipe rule | Allowed only after remix is finalized public | Same as any recipe | Remixer has normal owner permissions | Same as recipe/plan-snapshot rules; source creator receives no edit grant | Proposed: same as other recipes | Authenticated viewer may remix an accessible current finalized recipe; one operation replay is idempotent | Follows remix lifecycle and visibility; lineage source content and source-version attribution are independently authorized | Remixer alone; lineage and finalized attribution are immutable | Remixer alone | Follows normal recipe publication; source link/profile link is attribution, not shared authority | Denied; remix creation is independent copying, not transfer | Follows remix's own visibility; source deletion or source-owner deletion does not remove it; creator erasure nulls the internal reference | Remix lineage and exact source version use a minimized `recipe.remixed` event | DEC-012, DEC-013, DEC-018 | REC-11, REC-14, DEP-08 | Remix creator attribution follows its own finalized version. Accessible lineage may show the source version's selected label and enabled-profile link. Inaccessible lineage shows only an unavailable-source tombstone/version: no account name, email, internal ID, disabled profile, selected label, or inaccessible source title/link is exposed. Private-source remix remains owner-only under current access rules. |
| Bookmarks or saved recipes | Current private recipe bookmarks and REC-12 organisation | Bookmarking user | Bookmarking user | Owner only | Denied | Denied | Allowed: create, list, remove, and organize through owner-only REC-12 collections/private tags | Denied | Denied unless acting as owner | Authenticated user may idempotently bookmark a currently public finalized durable recipe; ownership is server-derived | Owner only; live recipe content is resolved separately through REC-06 | No content edit; REC-12 organization is owner only | Owner only; repeated removal is safe | Cannot share a bookmark; it is a pointer and copies no recipe content | Denied | Permanently removed with bookmark owner's private data; source deletion leaves the owner's bookmark tombstone | Not required for ordinary add/remove | DEC-012, DEC-013 | REC-10, REC-12, DEP-07, DEP-08 | The indexed durable recipe ID intentionally has no source foreign key. Private, missing, or deleted sources remain generic tombstones; re-publication restores existing pointers; active drafts never appear. |
| Recipe collections and private tags | Current REC-12 | Account user | Account user | Owner only | Denied | Denied | Allowed: create, view, edit, delete, organise owned recipes and own bookmarks, and filter by own private tags | Denied | Denied unless acting as owner | Authenticated user for own account; ownership is server-derived | Owner only | Owner only; membership targets must have the same owner | Owner only; membership rows are cleaned without deleting recipes/bookmarks | Denied; there are no public organization URLs | Denied; no transfer workflow exists | Permanently removed at final purge after any recovery period | Not required for ordinary organization changes | DEC-012, DEC-013 | REC-12, DEP-07, DEP-08 | Private tags are separate from public recipe tags. Foreign public recipes are organized through the user's own bookmark. Organization never grants recipe access, and REC-10 tombstones remain content-free inside collections/tags. Explicit public recipe, discovery, and REC-14 profile projections omit organization records and counts. |
| Public recipe tags and managed tag vocabularies | Current REC-13 | Free-form tag: recipe owner. Managed vocabulary: application | Free-form tag: recipe creator. Managed term: authorized administrator | Public only when attached to an accessible public recipe; management records administrator only | Allowed only as rendered public recipe metadata | Allowed on accessible recipes; managed vocabulary administration denied | Recipe owner may apply/remove allowed tags on own recipe; cannot alter managed definitions | Read-only as recipe metadata | Allowed: create/edit/deprecate managed vocabulary; proposed no direct retagging of user recipes | Owner creates free-form tags; administrator creates managed terms | Inherit recipe visibility; admin management view is administrator only | Owner edits own assignment; administrator edits managed definitions | Owner removes own assignment; administrator deprecates/removes managed term only with referential integrity | Tags do not grant access; public rendering inherits recipe visibility | Not applicable | Recipe tags follow retained/deleted recipe; managed vocabulary survives any contributor account | Managed-term changes and suggestion approval/rejection are auditable where moderation is involved | DEC-009, DEC-013 | REC-13, FND-04, FND-05 | Suggested claims require creator review and cannot be presented as verified with incomplete nutrition. |
| Current user-owned ingredients | Current legacy compatibility resource | `ingredients.user_id` user | Same user | Owner only | Denied by authenticated routes | Denied by owner policy/query scope | Create/view; edit/delete only while unmapped and not a verified import | Denied | No administrator role or override exists | Authenticated user; store paths assign authenticated user through trusted server-side association | Owner only through the compatibility path | Owner only while unmapped and not a verified import; mapped catalogue records and verified imports return 403 at controller and Livewire boundaries | Owner only while unmapped and not a verified import; hard delete | Denied | Denied; ownership is not mass assignable or included in mutation allowlists | Current database cascade removes the legacy row; mapped catalogue identity and migration evidence remain | No ingredient mutation audit event required by the current audit policy | None | STB-01, STB-03, FND-02, NUT-03 | This row documents the temporary legacy path. Mapping never grants ownership of the catalogue record. Restore and force-delete policy actions are denied. |
| Approved shared catalogue records and versions | Current NUT-03 read policy; later version payload planned | No user owner; application shared catalogue | Source import, submitter, or accepted proposal; submitter is provenance only | Public read-only | Allowed read-only when approved | Allowed read-only when approved | Not applicable as user ownership; submitter has no direct edit/delete right | Read-only; recipe/plan use does not grant mutation | Allowed: review staged changes, accept/reject them, confirm duplicate approved identities, explicitly select the canonical survivor, merge them, and initiate correction; direct silent mutation of current version denied | Successful barcode import or administrator approval creates/advances a record | Approved shared records allowed to everyone, including logged-out visitors | Ordinary users denied; changes occur by proposal and approved new version | Ordinary users denied. Administrator removal behavior for wrong/obsolete records is not yet specified | Cannot be published by a user; approval makes a pending manual record shared | Not applicable; catalogue is not user-owned | Record remains; submitting-user reference is anonymized or removed | Imports/source versions, accepted changes, duplicate candidates, merges, corrections, and provenance require audit/history | DEC-009, DEC-010, DEC-011, DEC-012, DEC-013 | NUT-01, NUT-03, NUT-05, NUT-06, NUT-09 through NUT-11, DEP-08 | Ordinary users cannot directly edit/delete barcode imports. Confirmed manual duplicates retain the source identity, versions, provenance, and direct canonical redirect; only declared live references move with reversible evidence. NUT-03 temporarily projects factual display fields through the explicit NUT-02 mapping without exposing provenance. |
| Pending manual catalogue additions | Current NUT-08 submission/read/use policy and NUT-09 administrator moderation | No ordinary-user owner; submitter has temporary private read/use access | Submitting user as nullable provenance | Submitter and administrator only | Denied | Denied unless the viewer is submitter | Allowed: create, view, and use while pending; edit, withdraw, delete, provenance reassignment, and self-approval denied | Denied | Current NUT-09: mark a candidate distinct, reject as duplicate with a suggested replacement, confirm duplicate evidence, approve, or reject; no decision without administrator authorization | Authenticated user submits a non-barcode item; barcode and client-controlled provenance/state are prohibited | Submitter and administrator only; rejected never-approved tombstone remains private | Submitter denied; administrator does not edit it as user content but records a moderation decision | Submitter withdrawal/delete denied; duplicate rejection retains a non-selectable tombstone and dependent references until owner-confirmed replacement or clearing | Cannot be shared/published; approval makes it shared catalogue data | Not applicable; approval changes catalogue status, not ownership transfer | Submitting-user deletion nulls provenance without deleting or publicizing the pending/tombstoned identity; approved record follows shared-catalogue retention | Submission and distinction continuation are auditable now; review, decision, and dependent-reference outcome require the corresponding workflow | DEC-009, DEC-010, DEC-011, DEC-013 | NUT-08, NUT-09, FND-04, FND-05, DEP-08 | DEC-011 prohibits automatic merging. Strong deterministic matches require explicit reuse or a bounded private distinction explanation; exact/approved-alias candidates are unordered and idempotent; fuzzy matches suggest only. Rejection never silently substitutes another item; only an editable resource owner may explicitly confirm an approved replacement. |
...
| Recipe import requests and retained source provenance | Current for pasted text, URL, document, photograph, and OCR channels | Importing user | Importing user or authorized job acting for user | Owner only; resulting recipe is a private draft | Denied | Denied | Allowed: submit pasted text, view own status/source/result, retry failed work within server bounds, and edit resulting draft; cancellation is not implemented | Denied | Denied content access; operational failure metadata is minimized and source-free | Authenticated user for own account | Owner only | Owner edits resulting draft, not extraction provenance | Request and private provenance follow draft and account purge; terminal operational metadata is deleted after 30 days | Cannot share or publish import request; only reviewed recipe can later be finalized/published | Denied | Private draft/provenance removed after recovery; terminal metadata follows the 30-day operational clock | Parser identity/version, correlation, outcome, warnings, and draft linkage are retained; source is excluded from audit/failure payloads | DEC-005, DEC-006, DEC-007, DEC-012, DEC-013 | REC-15 through REC-17, FND-09, DEP-08 | Accepted pasted source is preserved exactly, remains private, and is never placed in queue payloads. Imports never publish automatically. |
| OCR, document, photograph, and scan source files | Current transient resource | Uploading user | Uploading user | Owner and authorized processing service only | Denied | Denied | Allowed: upload for own import and view only where review requires it; download/reuse is not specified | Denied | Proposed: no content access; narrowly authorized security processing must be specified and minimized | Authenticated user uploads for own import, subject to validation | Owner/processing service only during processing; direct object URL access by others denied | Denied; replacement is a new upload | Automatically hard-deleted after success or failure; not retained as recipe attachment | Denied | Denied | Deleted after processing, normally before account deletion; deletion must also occur on failure | Creation, processing, and verified deletion event logged without raw file content | DEC-005, DEC-006, DEC-007, DEC-012, DEC-013 | REC-17, FND-09, DEP-03 | Provider choice/privacy remains unresolved. Transient deletion is confirmed; terminal files are deleted within 24 hours, abandoned files within seven days; backup behavior is not. |
| Import processing records and job metadata | Current for pasted-text, URL, document, photograph, and OCR processing | Importing user for access; application for operation | Application job acting for importing user | Owner only for user-facing status; operational details restricted | Denied | Denied | Allowed: view own status/result and invoke bounded retry; cancellation is not implemented | Denied | Minimized source-free operations metadata only; no private content | Created only for an authorized user's import | Owner sees own status and safe failure category | User cannot rewrite provenance/job history; retry advances controlled state on the same import | Terminal operational metadata is deleted 30 days after completion | Denied | Denied | User-facing record removed with import/account; minimal terminal operational metadata is deleted after 30 days under DEC-013 | Correlation, attempt/failure classification, parser identifier, and opaque import identity are recorded without input text | DEC-005, DEC-006, DEC-007, DEC-013 | FND-09, REC-15 through REC-17, DEP-04, DEP-05 | REC-15 uses identifier-only queue payloads and FND-09 safe reporting; upload channels also use identifier-only queue payloads. |
...
1. **Logged-out access:** Logged-out visitors may read only explicitly public
   ...
   are denied. Approved shared-catalogue records are explicitly public and
   read-only to logged-out visitors.
...
6. **Catalogue proposal review:** Pending manual additions are visible only to
   ...
   levels; decided DEC-011 governs duplicate candidates, rejection, approved
   merges, canonical redirects, references, and correction.
...
- Define amendment/withdrawal behavior for catalogue correction proposals,
  administrator removal behavior for obsolete catalogue data, and retention
  of unreferenced superseded recipe versions.
...
| Approved catalogue | Public read, including guests, and ordinary-user mutation denial; submitter receives no owner mutation; accepted proposal creates visible version; submitter deletion nulls/anonymizes provenance without deleting record. |
...
## REC-13 implemented access clarification

The `Public recipe tags and managed tag vocabularies` row is now current.
Recipe owners alone mutate their free-form tags and accepted classifications;
the central `access-admin` ability is enforced by route middleware, policies,
controller mutation boundaries, and the suggestion service for vocabulary
management and administrator-authored suggestions. Guests and ordinary users
cannot manage vocabulary. A suggestion grants no recipe access and exposes no
private recipe content; only the recipe owner reviews it. Pending/rejected
suggestions, private organisational tags, actors, and audit data are excluded
from public projections and REC-09 joins. Deactivation preserves existing
associations but prevents new attachment/suggestion/approval. No current
nutrition metadata is represented as verified.

## REC-14 implemented access clarification

The `Public attribution profiles` row is now current. Only the authenticated
account owner can create or change settings, and the mutation accepts no target
account/profile identifier. Guests and authenticated users share the same
read-only enabled-profile projection; disabled profiles are 404. Recipe and
remix lists reuse REC-06 public eligibility and expose only current immutable
finalized content. Email, private account/security fields, drafts, private and
historical recipes, active revisions, bookmarks, collections, private tags and
...
The OCR/document/photo resource is now the implemented REC-17 transient flow.
DEP-03 provides its reusable private-input boundary: non-served storage,
...
filename as a path or telemetry field. Jobs persist an opaque input
record identifier and never serialize the handle, bytes, source text, filename,
path or provider payload. REC-17 owns format-specific authorization, strict
document/image agreement and resource limits, local canonicalization/OCR,
field-specific review warnings, retention execution and cleanup verification.
Original and canonical files are deleted immediately where possible on every
terminal path, within 24 hours by the hourly safety sweep, or after seven days
