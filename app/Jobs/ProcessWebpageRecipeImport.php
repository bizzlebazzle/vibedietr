...
use App\Domain\RecipeImports\WebpageRecipeImportProcessor;
...
final class ProcessWebpageRecipeImport implements ShouldBeUnique, ShouldQueue
{
    use Queueable;

    public const OPERATION_TYPE = 'recipe_webpage_import.process';
    ...
    public function middleware(): array
    {
        return [
            ...
            (new WithoutOverlapping('recipe_webpage_import.global'))->releaseAfter(10)->expireAfter(75),
        ];
    }
    ...
    public function handle(WebpageRecipeImportProcessor $processor): void
    {
        ...
        try {
            ...
            if ($exception->retryable) {
                throw new RetryableJobException($exception->safeErrorCode);
            }
            ...
            throw RetryableJobException::fromUnexpected($exception, 'recipe_webpage_import_unexpected');
        }
    }

    public function failed(?Throwable $exception): void
    {
        $exception ??= new NonRetryableJobException('recipe_webpage_import_failed');
        $this->markFailed($exception);
