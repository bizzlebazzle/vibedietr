...
final class CatalogueModerationQueue
{
    public const STATES = [
        ...
        'provider_refresh' => ['queued', 'processing', 'staged', 'no_change', 'not_found', 'failed', 'cancelled', 'accepted', 'rejected'],
    ];

    /** @return LengthAwarePaginator<int, CatalogueItem>|LengthAwarePaginator<int, CatalogueDuplicateCandidate>|LengthAwarePaginator<int, CatalogueCorrectionProposal>|LengthAwarePaginator<int, CatalogueProviderRefresh> */
    public function paginate(User $actor, string $type, string $state = ''): LengthAwarePaginator
    {
        ...
        if ($type === 'correction_proposal') {
            return CatalogueCorrectionProposal::query()
                ->where('proposal_type', CatalogueChangeProposalType::UserCorrection)
                ->with(['catalogueItem.currentVersion'])
                ->when($state !== '', fn ($q) => $q->where('state', $state))
                ->orderBy('id')->paginate(25)->withQueryString();
        }

        if ($type === 'provider_refresh') {
            return CatalogueProviderRefresh::query()
                ->with(['catalogueItem.currentVersion', 'proposal'])
                ->when($state !== '', fn ($q) => $q->where('state', $state))
